...
@interface ProPlanLiuLanVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,UIAlertViewDelegate>
...
@implementation ProPlanLiuLanVC
...
- (void)creatUI{
    ...
    NSArray* titleArray = @[@"生产计划单号",@"计划日期",@"计划指定人",@"是否有效",@"备注"];
    NSString* planNo = [NSString stringWithFormat:@"%@",_model.planno];
    NSString* planDate = [NSString stringWithFormat:@"%@",_model.plantime];
    NSString* planPeople = [NSString stringWithFormat:@"%@",_model.creator];
    NSString* isValid;

    if ([_model.isvalid integerValue] == 1) {
         isValid = [NSString stringWithFormat:@"%@",@"是"];
    }else{
         isValid = [NSString stringWithFormat:@"%@",@"否"];
    }
   
    NSString* note = [NSString stringWithFormat:@"%@",_model.note];

    NSArray* detailArray = @[planNo,planDate,planPeople,isValid,note];
...
- (void)requestPifuHistory
{
    ...
    NSString *str =[NSString stringWithFormat:@"action=getAnswers&data={\"id\":\"%@\",\"table\":\"scjh\"}",_chanpinID];
    NSLog(@"审批历史字符串:%@",str);
...
- (void)spUPdata{
    ...
    if (status == 1 || status == -1||status == 11||status == 13||status == -2) {
         ...
         */
        NSString *urlStr = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/sp"];
        NSDictionary *params = @{@"action":@"doSp",@"table":@"scjh",@"data":[NSString stringWithFormat:@"{\"SPresult\":\"%zi\",\"SPcontent\":\"%@\",\"recordid\":\"%@\"}",status,_pifuneirong.text,_model.Id]};
        [DataPost requestAFWithUrl:urlStr params:params finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
         ...
         */
        NSString *urlStr = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/sp"];
        NSDictionary *params = @{@"action":@"doSp",@"table":@"scjh",@"data":[NSString stringWithFormat:@"{\"recordid\":\"%@\",\"tpaccountid\":\"%@\",\"tpname\":\"%@\",\"SPresult\":\"%zi\",\"SPcontent\":\"%@\"}",_model.Id,_tPID,_pifurenname.titleLabel.text,status,_pifuneirong.text]};
        [DataPost requestAFWithUrl:urlStr params:params finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
            NSString *str1 = [[NSString alloc]initWithData:result encoding:NSUTF8StringEncoding];
            NSLog(@"批复结果返回:%@",str1);
            NSString *realStr;
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    PPLiuLanDetailproCell* procell = [tableView dequeueReusableCellWithIdentifier:@"PPLiuLanDetailproCellID"];
    if (!procell) {
        procell = [[[NSBundle mainBundle]loadNibNamed:@"PPLiuLanDetailproCell" owner:self options:nil]firstObject];
    }
...
- (void)DataRequest2
{
    ...
    NSDictionary* parmas = @{@"mobile":@"true",@"action":@"getDetailPlanBeans",@"params":[NSString stringWithFormat:@"{\"idEQ\":\"%@\"}",_chanpinID]};
    ...
    [DataPost requestAFWithUrl:urlStr params:parmas finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
        ...
        if ([returnStr isEqualToString:@"sessionoutofdate"]) {
            ...
            _tbView.frame = CGRectMake(0, 10+45*6, KscreenWidth, 360*_dataArray.count+10);
            if ([self.vcType isEqualToString:@"1"]) {
                _spView.frame = CGRectMake(0, _tbView.bottom+10, KscreenWidth, 180+45*pifuLishicount);
                self.dingDanScrollView.contentSize = CGSizeMake(KscreenWidth, 10+45*6+360*_dataArray.count+10+20+_spView.height);
            }else{
                self.dingDanScrollView.contentSize = CGSizeMake(KscreenWidth, 10+45*6+360*_dataArray.count+10+20);
            }
            [_tbView reloadData];
            NSLog(@"产品信息%@",arr);
        }
