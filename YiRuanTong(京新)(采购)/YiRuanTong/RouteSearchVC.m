...
@interface RouteSearchVC ()<UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource>
...
//    [self.view addSubview:typeView];
    ...
    //
    UILabel *startLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, KscreenWidth/3, 50*MYWIDTH)];
    ...
    _startButton.frame = CGRectMake(KscreenWidth/3, 0, KscreenWidth/3*2, 50*MYWIDTH);
...
- (void)search{
    ...
    NSTimeInterval timeInterval = [dateB timeIntervalSinceDate:dateA];  //这个是相隔的秒数，除以3600就是小时数，再除以24就是天数，一次类推
    float day = timeInterval/3600/24;
    NSLog(@"%f",timeInterval/3600/24);
    if (day > 1) {
        [self showAlert:@"请选择一天内的日期"];
        return;
    }if (day < 0) {
        [self showAlert:@"请结束时间大于开始时间"];
        return;
    }
    if (_block) {
