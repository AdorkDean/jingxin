...
#import "CgInformationDetailVC.h"
#import "CgInformationSearchVC.h"
@interface CgInformationManageVC ()<UITextFieldDelegate>
...
@implementation CgInformationManageVC
...
- (void)navigationSetRightButton{
    ...
    [searchButton setTitle:@"申请" forState:UIControlStateNormal];
...
- (void)searchData
{
    ...
    if (_liuLanButton.selected == YES) {
        urlStr = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/purchase"];
        params = @{@"rows":@"10",@"page":[NSString stringWithFormat:@"%zi",_searchPage1],@"action":@"getBeans",@"table":@"cgd",@"params":[NSString stringWithFormat:@"{\"purchasenoLIKE\":\"%@\",\"timeGE\":\"%@\",\"timeLE\":\"%@\",\"isvalidEQ\":\"%@\"}",_orederNo,_start,_end,@"1"]};
        [DataPost requestAFWithUrl:urlStr params:params finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
            ...
            for (NSDictionary *dic in array) {
                CgLLManageModel *model = [[CgLLManageModel alloc] init];
        ...
        urlStr = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/purchase"];
        params = @{@"rows":@"10",@"page":[NSString stringWithFormat:@"%zi",_searchPage2],@"action":@"getPurchaseSP",@"table":@"cgd",@"params":[NSString stringWithFormat:@"{\"purchasenoLIKE\":\"%@\",\"timeGE\":\"%@\",\"timeLE\":\"%@\",\"spstatusEQ\":\"%@\"}",_orederNo,_start,_end,@"0"]};
        [DataPost requestAFWithUrl:urlStr params:params finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
            NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:result options:NSJSONReadingMutableContainers error:nil];
            NSArray *array = dic[@"rows"];
            NSLog(@"搜索上传字符串%@",params);
...
{
    //拼接地址字符串
    //http://192.168.1.199:8080/jingxin/servlet/purchase?action=getBeans&table=cgd
//    {"table":"cgd","isvalidEQ":"1","purchasenoLIKE":""，"timeGE":"","timeLE":""}
    NSString *urlStr = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/purchase"];
    NSDictionary *params = @{@"rows":@"10",@"page":[NSString stringWithFormat:@"%zi",_page1],@"action":@"getBeans",@"table":@"cgd",@"params":[NSString stringWithFormat:@"{\"purchasenoLIKE\":\"%@\",\"timeGE\":\"%@\",\"timeLE\":\"%@\",\"isvalidEQ\":\"%@\"}",@"",_pastDateStr,_currentDateStr,@"1"]};
    NSLog(@">>>>%@",urlStr);
    [DataPost requestAFWithUrl:urlStr params:params finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
...
{
    NSString *urlStr = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/purchase"];
    NSDictionary *params = @{@"rows":@"10",@"page":[NSString stringWithFormat:@"%zi",_page2],@"action":@"getPurchaseSP",@"table":@"cgd",@"params":[NSString stringWithFormat:@"{\"purchasenoLIKE\":\"%@\",\"timeGE\":\"%@\",\"timeLE\":\"%@\",\"spstatusEQ\":\"%@\"}",@"",_pastDateStr,_currentDateStr,@"0"]};
    [DataPost requestAFWithUrl:urlStr params:params finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CgInformationLLCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
    if (cell == nil) {
        cell = (CgInformationLLCell *) [[[NSBundle mainBundle] loadNibNamed:@"CgInformationLLCell" owner:self options:nil]firstObject];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == self.liuLanTableView)
    {
        ...
        CgInformationDetailVC *liuLanMessage =[[CgInformationDetailVC alloc] init];
        liuLanMessage.vcType = @"0";
        if (_searchFlag == 1) {
            if (_searchDateArray1.count != 0) {
                CgLLManageModel *model = [_searchDateArray1 objectAtIndex:indexPath.row];
                liuLanMessage.model = model;
//                liuLanMessage.wuliuID = model.logisticsid;
                liuLanMessage.chanpinID = model.Id;
                liuLanMessage.orderNo = model.purchaseno;
            }
        }else{
            if (_dataArray1.count != 0) {
                CgLLManageModel *model = [_dataArray1 objectAtIndex:indexPath.row];
                liuLanMessage.model = model;
//                liuLanMessage.wuliuID = model.logisticsid;
                liuLanMessage.chanpinID = model.Id;
                liuLanMessage.orderNo = model.purchaseno;
    ...
    {
        CgInformationDetailVC *shenPiMessage =[[CgInformationDetailVC alloc] init];
        shenPiMessage.vcType = @"1";
        if (_searchFlag == 1) {
            if (_searchDateArray2.count != 0) {
                CgLLManageModel *model = [_searchDateArray2 objectAtIndex:indexPath.row];
                shenPiMessage.model = model;
                shenPiMessage.spStatus = [model.spstatus integerValue];
                shenPiMessage.chanpinID = model.Id;
                shenPiMessage.orderNo = model.purchaseno;
            }
        }else{
            if (_dataArray2.count != 0) {
                CgLLManageModel *model = [_dataArray2 objectAtIndex:indexPath.row];
                shenPiMessage.model = model;
                shenPiMessage.spStatus = [model.spstatus integerValue];
                shenPiMessage.chanpinID = model.Id;
                shenPiMessage.orderNo = model.purchaseno;
...
- (void)addNext{
    CgInformationSearchVC *sousuoVC = [[CgInformationSearchVC alloc] init];
    if (_liuLanButton.selected == YES) {
        _type = @"isvalidEQ";
    }else{
        _type = @"spstatusEQ";
...
- (void)searchAction{
    CgApplyVC *tianjiaVC = [[CgApplyVC alloc] init];
