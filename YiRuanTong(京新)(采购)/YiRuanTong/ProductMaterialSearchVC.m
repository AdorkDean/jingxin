...
@interface ProductMaterialSearchVC ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate>
{
    MBProgressHUD *_HUD;
...
@implementation ProductMaterialSearchVC
...
- (void)createUIview{
    ...
    _bomName.frame = CGRectMake(KscreenWidth/3, 0, KscreenWidth/3*2, 50*MYWIDTH);
    ...
    _prostatusBtn.frame = CGRectMake(KscreenWidth/3, Label4.bottom +1, KscreenWidth*2/3, 50*MYWIDTH);
    [_prostatusBtn setTitleColor:UIColorFromRGB(0x999999) forState:UIControlStateNormal];
    [_prostatusBtn setTitle:@"请选择生产状态" forState:UIControlStateNormal];
...
- (void)search{
    NSString *stoname = _bomName.titleLabel.text;
    if ([stoname isEqualToString:@"请选择BOM单号"]||[stoname isEqualToString:@" "]) {
        stoname = @"";
    }
    ...
    if ([status isEqualToString:@"请选择生产状态"]||[status isEqualToString:@" "]) {
        status = @"";
    }else if ([status isEqualToString:_statusArray[0]]){
        status = @"0";
    }else if ([status isEqualToString:_statusArray[1]]){
        status = @"1";
    }else if ([status isEqualToString:_statusArray[2]]){
        status = @"2";
    }
    ...
    if (_block) {
        self.block(stoname, planname, status, nameid);
    }
...
- (void)statusAction{
    ...
    UIAlertAction * okAction2 = [UIAlertAction actionWithTitle:_statusArray[2] style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
         [_prostatusBtn setTitle:_statusArray[2] forState:UIControlStateNormal];
    }];
