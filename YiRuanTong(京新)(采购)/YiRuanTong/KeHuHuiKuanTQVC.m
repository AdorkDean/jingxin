...
#import "CustCell.h"
@interface KeHuHuiKuanTQVC ()<UITextFieldDelegate>{
...
@implementation KeHuHuiKuanTQVC
...
- (void)initTableView{
    self.keHuHuiKuaiTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 50, KscreenWidth, KscreenHeight-114) style:UITableViewStylePlain];
    self.keHuHuiKuaiTableView.delegate = self;
    self.keHuHuiKuaiTableView.dataSource = self;
//    _refreshControl = [[UIRefreshControl alloc] init];
//    _refreshControl.attributedTitle = [[NSAttributedString alloc] initWithString:@"开始刷新..."];
//    [_refreshControl addTarget:self action:@selector(refreshData) forControlEvents:UIControlEventValueChanged];
//    [self.keHuHuiKuaiTableView addSubview:_refreshControl];
    [self.view addSubview:_keHuHuiKuaiTableView];
    //     下拉刷新
    _keHuHuiKuaiTableView.mj_header= [HTRefreshGifHeader headerWithRefreshingBlock:^{
        [self refreshDown];
        // 结束刷新
        [_keHuHuiKuaiTableView.mj_header endRefreshing];
        
    }];
    // 设置自动切换透明度(在导航栏下面自动隐藏)
    _keHuHuiKuaiTableView.mj_header.automaticallyChangeAlpha = YES;
    // 上拉刷新
    _keHuHuiKuaiTableView.mj_footer = [HTRefreshGifFooter footerWithRefreshingBlock:^{
        [self upRefresh];
        [_keHuHuiKuaiTableView.mj_footer endRefreshing];
    }];
}
...
{/*
  ...
  params	{"starttime":"2015-02-05","endtime":"2015-03-05"}*/
    ...
    NSDictionary *params = @{@"tjlx":@"detail",@"mobile":@"true",@"action":@"custReMoneyInSameTime",@"page":[NSString stringWithFormat:@"%zi",_page],@"rows":@"20",@"params":[NSString stringWithFormat:@"{\"starttime\":\"%@\",\"endtime\":\"%@\"}",_pastDateStr,_currentDateStr]};
    [DataPost requestAFWithUrl:urlStr params:params finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
        ...
        if (array.count != 0) {
            for (NSDictionary *dic  in array) {
                KeHuHuiKuanModel *model = [[KeHuHuiKuanModel alloc] init];
                [model setValuesForKeysWithDictionary:dic];
                [_DataArray addObject:model];
                NSString *str = [dic objectForKey:@"custname"];
                [_nameArray addObject:str];
                NSString *str1 = [dic objectForKey:@"returnmoney"];
                [_thisYearArray addObject:str1];
                NSString *str2 = [dic objectForKey:@"lastyear"];
                [_lastYearArray addObject:str2];
            }
            [self.keHuHuiKuaiTableView reloadData];
            NSLog(@"客户回款同期对比:%@",dic);
...
{/*
  ...
  */
    ...
    NSString *urlStr = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/report"];
    NSDictionary *params = @{@"action":@"custReMoneyInSameTime",@"tjlx":@"total",@"mobile":@"true",@"params":[NSString stringWithFormat:@"{\"starttime\":\"%@\",\"endtime\":\"%@\"}",_pastDateStr,_currentDateStr]};
    ...
    [DataPost requestAFWithUrl:urlStr params:params finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
        ...
        if (array.count != 0) {
            NSString *lastallreturned =[NSString stringWithFormat:@"去年:%@",array[0][@"lastallreturned"]];
            self.m_quNian.text =lastallreturned;
            NSString *allreturned =[NSString stringWithFormat:@"今年:%@",array[0][@"allreturned"]];
...
-(void)PageViewDidLoad
{
    ...
    self.m_quNian =[[UILabel alloc]initWithFrame:CGRectMake(15, 5, KscreenWidth/2 - 15, 40)];
    ...
    self.m_jinNian =[[UILabel alloc]initWithFrame:CGRectMake(KscreenWidth/2 ,5, KscreenWidth/2 - 15, 40)];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    KeHuHuiKuanCell *cell =[tableView dequeueReusableCellWithIdentifier:cellID];
    if (cell ==nil) {
        cell =(KeHuHuiKuanCell*)[[[NSBundle mainBundle]loadNibNamed:@"KeHuHuiKuanCell" owner:self options:nil]firstObject];
    }
    ...
    if (tableView ==self.keHuHuiKuaiTableView) {
        if (_searchFlag == 1) {
...
{
    if (tableView == self.keHuHuiKuaiTableView) {
        KeHuHuiKuanChartView *chartView = [[KeHuHuiKuanChartView alloc] init];
        if (_searchFlag == 1) {
            ...
            chartView.countData = @[_thisYearArray[indexPath.row]];
            chartView.moneyData = @[_lastYearArray[indexPath.row]];
...
- (void)searchData
{
    ...
    NSString *urlStr = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/report"];
    NSDictionary *params  = @{@"rows":@"20",@"tjlx":@"detail",@"page":[NSString stringWithFormat:@"%zi",_searchPage],@"action":@"custReMoneyInSameTime",@"tjlx":@"detail",@"params":[NSString stringWithFormat:@"{\"custidEQ\":\"%@\",\"departidEQ\":\"\",\"custnameLIKE\":\"%@\",\"starttime\":\"%@\",\"endtime\":\"%@\"}",_salerId,cust,start,end]};
    [DataPost requestAFWithUrl:urlStr params:params finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
...
{/*
  ...
  */
    ...
    NSString *urlStr = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/report"];
    NSDictionary *params = @{@"action":@"custReMoneyInSameTime",@"tjlx":@"total",@"mobile":@"true",@"params":[NSString stringWithFormat:@"{\"custidEQ\":\"%@\",\"departidEQ\":\"\",\"custnameLIKE\":\"%@\",\"starttime\":\"%@\",\"endtime\":\"%@\"}",_salerId,cust,start,end]};
...
- (void)searchAction{
    ...
    if (_searchFlag == 1) {
        ...
        chartView.countData = _thisYearArray;
        chartView.moneyData = _lastYearArray;
    }
    [self.navigationController pushViewController:chartView animated:YES];
