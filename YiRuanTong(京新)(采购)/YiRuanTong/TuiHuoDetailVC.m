...
@interface TuiHuoDetailVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,UIAlertViewDelegate>
...
- (void)initScrollView {
    ...
    if ([self.vcType isEqualToString:@"0"]) {
        ...
        [_stateButton setTitle:@"图片" forState:UIControlStateNormal];
...
- (void)creatUI{
    UIView* headerView = [[UIView alloc]initWithFrame:CGRectMake(0, 10, KscreenWidth, 45*8)];
    ...
    NSArray* titleArray = @[@"退货单号",@"创建时间",@"客户名称",@"金额",@"业务员",@"审批状态",@"退货原因",@"备注"];
    NSString* returnno = [NSString stringWithFormat:@"%@",_model.returnno];
    NSString* applytime = [NSString stringWithFormat:@"%@",_model.applytime];
    NSString* custname = [NSString stringWithFormat:@"%@",_model.custname];
    NSString* totalmoney = [NSString stringWithFormat:@"%@",_model.totalmoney];
    totalmoney = [NSString stringWithFormat:@"￥%.2f",[totalmoney doubleValue]];
    NSString* saler = [NSString stringWithFormat:@"%@",_model.saler];
    NSString* spnodename = [NSString stringWithFormat:@"%@",_model.spnodename];
    NSString* returnreason = [NSString stringWithFormat:@"%@",_model.returnreason];
    NSString* note = [NSString stringWithFormat:@"%@",_model.note];
    NSArray* detailArray = @[returnno,applytime,custname,totalmoney,saler,spnodename,returnreason,note];
...
- (void)shenPiload
{
    ...
    pifu.frame = CGRectMake(40, 44 + 45*(pifuLishicount+2)+10, 75, 30);
    ...
    chongzhi.frame = CGRectMake(40+60+75, 44 + 45*(pifuLishicount+2)+10, 75, 30);
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    TuiHuoDetailProCell* procell = [tableView dequeueReusableCellWithIdentifier:@"TuiHuoDetailProCellID"];
    if (!procell) {
        procell = [[[NSBundle mainBundle]loadNibNamed:@"TuiHuoDetailProCell" owner:self options:nil]firstObject];
    }
...
- (void)DataRequest2
{
    ...
    NSDictionary* parmas = @{@"mobile":@"true",@"action":@"toUpdateReturn",@"table":@"thmx",@"params":[NSString stringWithFormat:@"{\"idEQ\":\"%@\"}",_model.Id]};
    ...
    [DataPost requestAFWithUrl:urlStr params:parmas finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
        ...
        if ([returnStr isEqualToString:@"sessionoutofdate"]) {
            ...
            for (NSDictionary *dic in  arr) {
                TuiHuoDetailModel *model = [[TuiHuoDetailModel alloc] init];
            ...
            _tbView.frame = CGRectMake(0, 10+45*8, KscreenWidth, 250*_dataArray.count);
            if ([self.vcType isEqualToString:@"1"]) {
                _spView.frame = CGRectMake(0, _tbView.bottom+10, KscreenWidth, 180+45*pifuLishicount);
                self.dingDanScrollView.contentSize = CGSizeMake(KscreenWidth, 10+45*8+250*_dataArray.count+20+_spView.height);
            }else{
                self.dingDanScrollView.contentSize = CGSizeMake(KscreenWidth, 10+45*8+250*_dataArray.count+20);
            }
            [_tbView reloadData];
            NSLog(@"退货信息%@",arr);
        }
