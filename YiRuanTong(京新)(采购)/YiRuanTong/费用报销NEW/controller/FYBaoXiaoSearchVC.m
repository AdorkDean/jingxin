...
#import "CustCell.h"
@interface FYBaoXiaoSearchVC ()<UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource>
...
@implementation FYBaoXiaoSearchVC
...
- (void)search{
    NSString* idEQ;NSString* start;NSString* end;
    if ([_salerID isEqualToString:@" "]) {
        idEQ = @"";
    }else{idEQ = _salerID;};
    if ([_startButton.titleLabel.text isEqualToString:@" "]) {
        start = @"";
    }else{start = _startButton.titleLabel.text;}
    if ([_endButton.titleLabel.text isEqualToString:@" "]) {
        end = @"";
    }else{end = _endButton.titleLabel.text;}
    if (_block) {_block(idEQ,start,end);
    }
...
- (void)searchcust:(UIButton *)but{
    ...
    NSDictionary *params = @{@"action":@"getPrincipals",@"table":@"yhzh",@"rows":@"20",@"page":[NSString stringWithFormat:@"%zi",_custpage],@"mobile":@"true",@"params":[NSString stringWithFormat:@"{\"nameLIKE\":\"%@\"}",custName]};
    if (_custpage == 1) {
        [_dataArray removeAllObjects];
    }
    NSLog(@"par>>>%@",params);
...
- (void)searchsaler{
    ...
    NSString *urlstr = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/account"];
    NSDictionary *params = @{@"action":@"getPrincipals",@"table":@"yhzh",@"rows":@"20",@"flag":@"1",@"page":[NSString stringWithFormat:@"%zi",_salerpage],@"mobile":@"true",@"params":[NSString stringWithFormat:@"{\"nameLIKE\":\"%@\"}",saler]};
    if (_salerpage == 1) {
