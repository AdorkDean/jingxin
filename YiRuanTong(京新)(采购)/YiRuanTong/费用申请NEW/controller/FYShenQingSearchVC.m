...
#import "FYShenQingSearchVC.h"
#import "UIViewExt.h"
#import "DataPost.h"
#import "CommonModel.h"
#import "CustCell.h"
@interface FYShenQingSearchVC ()<UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource>
...
//    _orderTextfield.placeholder = @"请输入上报人";  // 提示文本
...
//    [self.view addSubview:nameView];
    //
        UILabel *custLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, KscreenWidth/3, 50*MYWIDTH)];
        custLabel.text = @"申请人";
        ...
        [_custButton setTitle:@"请选择申请人" forState:UIControlStateNormal];
        ...
        [_salerButton setTitle:@"请选择申请类型" forState:UIControlStateNormal];
    ...
    UILabel* spLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, typeView.bottom, KscreenWidth/3, 50*MYWIDTH)];
    spLabel.text = @"审批状态";
    spLabel.textColor = salerLabel.textColor;
    spLabel.backgroundColor = salerLabel.backgroundColor;
    spLabel.font = salerLabel.font;
    spLabel.textAlignment = salerLabel.textAlignment;
    _spstatusBtn = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    _spstatusBtn.frame = CGRectMake(KscreenWidth/3, salerLabel.bottom, KscreenWidth/3*2, 50*MYWIDTH);
    [_spstatusBtn setTitle:@"否" forState:UIControlStateNormal];
    [_spstatusBtn setTitleColor:UIColorFromRGB(0x999999) forState:UIControlStateNormal];
    [_spstatusBtn addTarget:self action:@selector(spBtnClick) forControlEvents:UIControlEventTouchUpInside];
    UIView* spView = [[UIView alloc]initWithFrame:CGRectMake(0, spLabel.bottom, KscreenWidth, 1)];
    spView.backgroundColor = typeView.backgroundColor;
    //
    UILabel *startLabel = [[UILabel alloc] init];
    if ([self.vctype isEqualToString:@"1"]) {
        [self.view addSubview:spLabel];
        [self.view addSubview:spView];
        [self.view addSubview:_spstatusBtn];
        startLabel.frame = CGRectMake(0, spView.bottom, KscreenWidth/3, 50*MYWIDTH);
    }else{
        startLabel.frame = CGRectMake(0, typeView.bottom, KscreenWidth/3, 50*MYWIDTH);
    }
    ...
    _startButton.frame = CGRectMake(KscreenWidth/3, startLabel.top, KscreenWidth/3*2, 50*MYWIDTH);
...
- (void)search{
    NSString* applyor;NSString* start;NSString* end;NSString* type;NSString* spstatus;
    if ([_custButton.titleLabel.text isEqualToString:@" "]||[_custButton.titleLabel.text isEqualToString:@"请选择申请人"]) {
        applyor = @"";
    }else{applyor = [NSString stringWithFormat:@"%@",_custId];}
    if ([_salerButton.titleLabel.text isEqualToString:@" "]||[_salerButton.titleLabel.text isEqualToString:@"请选择申请类型"]) {
        type = @"";
    }else{type = [NSString stringWithFormat:@"%@",_salerID];}
    if ([_startButton.titleLabel.text isEqualToString:@" "]||[_startButton.titleLabel.text isEqualToString:@""]) {
        start = @"";
    }else{start = _startButton.titleLabel.text;}
    if ([_endButton.titleLabel.text isEqualToString:@" "]||[_endButton.titleLabel.text isEqualToString:@""]) {
        end = @"";
    }else{end = _endButton.titleLabel.text;}
    if ([_spstatusBtn.titleLabel.text isEqualToString:@" "]||[_spstatusBtn.titleLabel.text isEqualToString:@"请选择审批状态"]||[_spstatusBtn.titleLabel.text isEqualToString:@"否"]) {
        spstatus = @"0";
    }else{spstatus = @"1";}
    if (_block) {
        _block(applyor,type,spstatus,start,end);
    }
...
////    [bgView addSubview:btn];
    ...
    if (self.salerTableView == nil) {
        self.salerTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 35,bgView.width, bgView.height - 35) style:UITableViewStylePlain];
    }
...
- (void)spBtnClick{
    UIAlertController * alertController = [UIAlertController alertControllerWithTitle:@"审批状态：" message:@"" preferredStyle:UIAlertControllerStyleActionSheet];
    ...
    UIAlertAction * okAction = [UIAlertAction actionWithTitle:_statusArray[0] style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [_spstatusBtn setTitle:_statusArray[0] forState:UIControlStateNormal];
    }];
    ...
    UIAlertAction * okAction1 = [UIAlertAction actionWithTitle:_statusArray[1] style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [_spstatusBtn setTitle:_statusArray[1] forState:UIControlStateNormal];
    }];
...
- (void)searchcust:(UIButton *)but{
    ...
    NSDictionary *params = @{@"action":@"getPrincipals",@"table":@"yhzh",@"flag":@"1",@"rows":@"20",@"page":[NSString stringWithFormat:@"%zi",_custpage],@"mobile":@"true",@"params":[NSString stringWithFormat:@"{\"nameLIKE\":\"%@\"}",custName]};
    if (_custpage == 1) {
        [_dataArray removeAllObjects];
    }
    NSLog(@"par>>>%@",params);
...
- (void)searchsaler{
    ...
    NSDictionary *params = @{@"action":@"getSelectType",@"type":@"costtype"};
    [DataPost requestAFWithUrl:urlstr params:params finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
