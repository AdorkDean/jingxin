...
@interface FYShenQingListVC ()<UITextFieldDelegate>
...
@implementation FYShenQingListVC
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"费用申请";
    ...
    [self DataRequest];
    [self DataRequest1];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(newRefresh) name:@"FYShenQingListVCnewRefresh" object:nil];
...
- (void)searchData
{
    ...
    _applyType = [self convertNull:_applyType];
    ...
    if (_liuLanButton.selected == YES) {
        ...
        NSDictionary *params = @{@"action":@"getBeans",@"table":@"fysq",@"rows":@"20",@"page":[NSString stringWithFormat:@"%zi",_searchPage1],@"params":[NSString stringWithFormat:@"{\"applyeridEQ\":\"%@\",\"costtypeidEQ\":\"%@\",\"applytimeGE\":\"%@\",\"applytimeLE\":\"%@\"}",_orederNo,_applyType,_start,_end]};
         ...
         */
        urlStr = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/costapply?action=getSPBeans"];
        params = @{@"rows":@"10",@"page":[NSString stringWithFormat:@"%zi",_searchPage2],@"action":@"getSPBeans",@"table":@"fysq",@"params":[NSString stringWithFormat:@"{\"applyeridEQ\":\"%@\",\"costtypeidEQ\":\"%@\",\"isspEQ\":\"%@\",\"applytimeGE\":\"%@\",\"applytimeLE\":\"%@\"}",_orederNo,_applyType,_spEQ,_start,_end]};
        [DataPost requestAFWithUrl:urlStr params:params finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
            NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:result options:NSJSONReadingMutableContainers error:nil];
            NSArray *array = dic[@"rows"];
            NSLog(@"搜索上传字符串%@",params);
...
- (void)DataRequest
{
    ...
    [hud show:YES];
    NSString *urlStr = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/costapply?action=getBeans"];
    NSDictionary *params = @{@"rows":@"10",@"page":[NSString stringWithFormat:@"%zi",_page1],@"table":@"fysq",@"params":[NSString stringWithFormat:@"{\"applyeridEQ\":\"\",\"costtypeidEQ\":\"\",\"applytimeGE\":\"\",\"applytimeLE\":\"\"}"]};
    NSLog(@">>>>%@",urlStr);
    [DataPost requestAFWithUrl:urlStr params:params finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
        if (_page1 == 1) {
...
- (void)DataRequest1
{
    ...
    NSDictionary *params = @{@"action":@"getSPBeans",@"table":@"fysq",@"rows":@"20",@"page":[NSString stringWithFormat:@"%zi",_page2],@"params":@"{\"applyeridEQ\":\"\",\"costtypeidEQ\":\"\",\"applytimeGE\":\"\",\"applytimeLE\":\"\",\"isspEQ\":\"0\"}"};
    [DataPost requestAFWithUrl:urlStr params:params finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
        if (_page2 == 1) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    FYShenQingTableCell *cell = [tableView dequeueReusableCellWithIdentifier:@"FYShenQingTableCellID"];
    if (cell == nil) {
        cell = (FYShenQingTableCell *) [[[NSBundle mainBundle] loadNibNamed:@"FYShenQingTableCell" owner:self options:nil]firstObject];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == self.liuLanTableView)
    {
        FYShenQingDetailVC *liuLanMessage =[[FYShenQingDetailVC alloc] init];
        liuLanMessage.vcType = @"0";
        if (_searchFlag == 1) {
            if (_searchDateArray1.count != 0) {
                FYSQModel *model = [_searchDateArray1 objectAtIndex:indexPath.row];
    ...
    {
        FYShenQingDetailVC *shenPiMessage =[[FYShenQingDetailVC alloc] init];
        shenPiMessage.vcType = @"1";
        if (_searchFlag == 1) {
            if (_searchDateArray2.count != 0) {
                FYSQModel *model = [_searchDateArray2 objectAtIndex:indexPath.row];
...
- (void)addNext{
    FYShenQingSearchVC* vc = [[FYShenQingSearchVC alloc]init];
    if (_liuLanButton.selected == YES) {
        vc.vctype = @"0";
    }else{
        vc.vctype = @"1";
    }
    [vc setBlock:^(NSString *orederNo, NSString *applytype, NSString *spEQ, NSString *start, NSString *end) {
        _orederNo = orederNo;
...
- (void)searchAction{
    ...
    FYSQShangBaoViewController *FYshangBao = [[FYSQShangBaoViewController alloc] init];
