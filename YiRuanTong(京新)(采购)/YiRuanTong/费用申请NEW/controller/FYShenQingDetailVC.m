...
@interface FYShenQingDetailVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,UIAlertViewDelegate>
...
@implementation FYShenQingDetailVC
...
- (void)creatUI{
    ...
    NSArray* titleArray = @[@"申请人",@"申请编号",@"申请时间",@"费用类型",@"申请原因",@"申请金额",@"审批状态",@"备注"];
    NSString* sono = [NSString stringWithFormat:@"%@",self.model.applyer];
    NSString* bom = [NSString stringWithFormat:@"%@",self.model.costno];
    NSString* planno = [NSString stringWithFormat:@"%@",self.model.applytime];
    NSString* proname = [NSString stringWithFormat:@"%@",self.model.costtype];
    NSString* creat = [NSString stringWithFormat:@"%@",self.model.applyaim];
    NSString* plancount = [NSString stringWithFormat:@"%@",self.model.applymoney];
    NSString* sp = [NSString stringWithFormat:@"%@",self.model.spnodename];
    NSString* note = [NSString stringWithFormat:@"%@",self.model.note];
    NSArray* detailArray = @[sono,bom,planno,proname,creat,plancount,sp,note];
...
- (void)requestPifuHistory
{
    ...
    NSString *str =[NSString stringWithFormat:@"action=getAnswers&data={\"id\":\"%@\",\"table\":\"fysq\"}",_chanpinID];
    NSLog(@"审批历史字符串:%@",str);
...
- (void)DataRequest2
{
    ...
    [DataPost requestAFWithUrl:urlStr params:parmas finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
        ...
        if ([returnStr isEqualToString:@"sessionoutofdate"]) {
            ...
            _tbView.frame = CGRectMake(0, 10+45*9, KscreenWidth, 315*_dataArray.count+10);
            if ([self.vcType isEqualToString:@"1"]) {
                _spView.frame = CGRectMake(0, _tbView.bottom+10, KscreenWidth, 180+45*pifuLishicount);
                self.dingDanScrollView.contentSize = CGSizeMake(KscreenWidth, 10+45*9+315*_dataArray.count+10+20+_spView.height);
            }else{
                self.dingDanScrollView.contentSize = CGSizeMake(KscreenWidth, 10+45*9+315*_dataArray.count+10+20);
            }
            [_tbView reloadData];
            NSLog(@"产品信息%@",arr);
        }
