...
@interface ChanPinXiaoShouSearchVC ()
...
@implementation ChanPinXiaoShouSearchVC
{
...
- (void)createUIview{
    ...
    [_areaButton setTitle:@"请选择区域" forState:UIControlStateNormal];
...
- (void)search{
    ...
    if ([_saler isEqualToString:@"请选择产品"]) {
        _saler = @"";
    }
    NSString *_area = _areaButton.titleLabel.text;
    if ([_area isEqualToString:@"请选择区域"]) {
        _area = @"";
    }
    if (_block) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_dataArray.count!=0) {
        cell1.model = _dataArray[indexPath.row];
    }
    return cell1;
