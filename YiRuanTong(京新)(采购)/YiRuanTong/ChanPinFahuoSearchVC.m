...
#import "CustCell.h"
#import "THCPModel.h"
@interface ChanPinFahuoSearchVC ()<UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource>
...
@implementation ChanPinFahuoSearchVC
{
...
- (void)searchpro{
    ...
    NSDictionary *params = @{@"action":@"getMBProduct",@"rows":@"20",@"page":[NSString stringWithFormat:@"%zi",_salerpage],@"table":@"cpxx",@"mobile":@"true",@"params":[NSString stringWithFormat:@"{\"proname\":\"%@\"}",proName]};
    if (_salerpage == 1) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == self.salerTableView){
        ...
        if (_salerArray.count != 0) {
            ...
            THCPModel *model = _salerArray[indexPath.row];
            cell2.textLabel.text = model.proname;
        }
        return cell2;
    }
