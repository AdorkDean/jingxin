...
#import "CustModel.h"
@interface KHCPXiaoShouSearchVC ()
...
@implementation KHCPXiaoShouSearchVC
{
...
- (void)createUIview{
    ...
    [_salerButton setTitle:@"请选择客户" forState:UIControlStateNormal];
...
- (void)search{
    ...
    if ([_saler isEqualToString:@"请选择名称"]) {
        _saler = @"";
    }
...
- (void)searchpro{
    ...
    NSDictionary *params = @{@"rows":@"20",@"page":[NSString stringWithFormat:@"%zi",_propage],@"table":@"fhxx",@"params":[NSString stringWithFormat:@"{\"proname\":\"%@\"}",pro]};
...
//
//        NSMutableArray * arr;
////        NSDictionary *dictt =[NSJSONSerialization JSONObjectWithData:result options:NSJSONReadingAllowFragments error:nil];
////
////        if([[dictt allKeys] containsObject:@"rows"])
////
////        {
////
////             arr = dictt[@"rows"];
...
////        }else{
//
//            arr =[NSJSONSerialization JSONObjectWithData:result options:NSJSONReadingAllowFragments error:nil];
//
////        }
