...
@interface ProPlanSearchViewController ()<UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource>
...
@implementation ProPlanSearchViewController
{
...
//    [_orderTextfield setValue:[UIFont systemFontOfSize:14.0] forKeyPath:@"_placeholderLabel.font"];
    ...
    [_orderTextBtn setTitle:@"请点击生产计划单号" forState:UIControlStateNormal];
    [_orderTextBtn addTarget:self action:@selector(salerAction) forControlEvents:UIControlEventTouchUpInside];
...
- (void)search{
    if (_block) {
        NSLog(@"%@,%@,%@",_orderTextBtn.titleLabel.text,_startButton.titleLabel.text,_endButton.titleLabel.text);
        NSString* _orderstr = [self convertNullkongge:_orderTextBtn.titleLabel.text];
        if ([_orderstr isEqualToString:@"请点击生产计划单号"]) {
            _orderstr = @"";
        }
