...
@interface CustomerServiceSearchVC ()<UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource>
...
@implementation CustomerServiceSearchVC

{
    UIButton* _orderTextBtn;
...
- (void)createUIview{
    ...
    //    _orderTextfield.placeholder = @"输入生产计划单号";  // 提示文本
    ...
    _orderTextBtn = [UIButton buttonWithType:UIButtonTypeSystem];
    _orderTextBtn.frame = CGRectMake(KscreenWidth/3, 0, KscreenWidth/3*2, 50*MYWIDTH);
    [_orderTextBtn setTitleColor:UIColorFromRGB(0x999999) forState:UIControlStateNormal];
    [_orderTextBtn setTitle:@"请点击是否处理" forState:UIControlStateNormal];
    [_orderTextBtn addTarget:self action:@selector(isDealClick:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [self.view addSubview:_orderTextBtn];
    ...
    //
    UILabel *startLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, nameView.bottom, KscreenWidth/3, 50*MYWIDTH)];
    ...
    _startButton.frame = CGRectMake(KscreenWidth/3, nameView.bottom, KscreenWidth/3*2, 50*MYWIDTH);
...
- (void)search{
    if (_block) {
        NSString* status ;
        if ([_orderTextBtn.titleLabel.text isEqualToString:@"是"]) {
            status = @"1";
        }else if([_orderTextBtn.titleLabel.text isEqualToString:@"否"]){
            status = @"0";
        }else {
            status = @"";
        }
        NSLog(@"%@,%@,%@",status,_startButton.titleLabel.text,_endButton.titleLabel.text);
        NSString* _start = [self convertNullkongge:_startButton.titleLabel.text];
        NSString* _end = [self convertNullkongge:_endButton.titleLabel.text];
        _block(status,_start,_end);
    }
...
- (void)salerAction{
    ...
    [bgView addSubview:btn];
    //    UILabel* nameLabel = [[UILabel alloc]initWithFrame:CGRectMake(10, 0, bgView.width - 20, 40)];
    //    nameLabel.text = @"生产计划单号";
    //    nameLabel.font = [UIFont systemFontOfSize:15];
    //    [bgView addSubview:nameLabel];
    
    if (self.salerTableView == nil) {
        self.salerTableView = [[UITableView alloc]initWithFrame:CGRectMake(10, _salerField.bottom+5,bgView.width-40, bgView.height - _salerField.bottom - 5) style:UITableViewStylePlain];////nameLabel.bottom+5
    }
...
- (void)isDealClick:(UIButton*)sender{
    ...
    UIAlertAction * okAction = [UIAlertAction actionWithTitle:_statusArray[0] style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [_orderTextBtn setTitle:_statusArray[0] forState:UIControlStateNormal];
    }];
    ...
    UIAlertAction * okAction1 = [UIAlertAction actionWithTitle:_statusArray[1] style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [_orderTextBtn setTitle:_statusArray[1] forState:UIControlStateNormal];
    }];
...
- (void)chongZhi
{
    ...
    [_orderTextBtn setTitle:@" " forState:UIControlStateNormal];
...
- (void)searchsaler{
    ...
    NSDictionary *params = @{@"rows":@"20",@"page":[NSString stringWithFormat:@"%zi",_salerpage],@"mobile":@"true",@"params":[NSString stringWithFormat:@"{\"nameLIKE\":\"%@\"}",saler]};
    if (_salerpage == 1) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == self.custTableView){
        ...
        if (_salerArray.count!=0) {
            ...
            //            _salerButton.userInteractionEnabled = YES;
            [_orderTextBtn setTitle:model.name forState:UIControlStateNormal];
            //            _salerID = [NSString stringWithFormat:@"%@",model.Id];
            _orderTextBtn.userInteractionEnabled = YES;
        }
    }
    [self.m_keHuPopView removeFromSuperview];
}
