...
@interface CgInformationDetailVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,UIAlertViewDelegate>
...
@implementation CgInformationDetailVC
...
- (void)creatUI{
    UIView* headerView = [[UIView alloc]initWithFrame:CGRectMake(0, 10, KscreenWidth, 45*9)];
    ...
    NSArray* titleArray = @[@"采购单号",@"采购部门",@"采购员",@"采购区域",@"付款方式",@"供应商",@"采购日期",@"备注"];
    NSString* purchaseno = [NSString stringWithFormat:@"%@",_model.purchaseno];
    NSString* departname = [NSString stringWithFormat:@"%@",_model.departname];
    NSString* accountname = [NSString stringWithFormat:@"%@",_model.accountname];
    NSString* placename = [NSString stringWithFormat:@"%@",_model.placename];
    NSString* paytype = [NSString stringWithFormat:@"%@",_model.paytype];
    NSString* suppliername = [NSString stringWithFormat:@"%@",_model.suppliername];
    NSString* createtime = [NSString stringWithFormat:@"%@",_model.createtime];
    NSString* note = [NSString stringWithFormat:@"%@",_model.note];

    NSArray* detailArray = @[purchaseno,departname,accountname,placename,paytype,suppliername,createtime,note];
...
- (void)requestPifuHistory
{
    ...
    NSString *str =[NSString stringWithFormat:@"action=getAnswers&data={\"id\":\"%@\",\"table\":\"cgd\"}",_chanpinID];
    NSLog(@"审批历史字符串:%@",str);
...
{
    //审批
    _spView = [[UIView alloc]initWithFrame:CGRectMake(0, _tbView.bottom, KscreenWidth, 200+45*pifuLishicount)];
...
- (void)spUPdata{
    ...
    if (status == 1 || status == -1||status == 11||status == 13||status == -2) {
         ...
         */
        NSString *urlStr = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/sp"];
        NSDictionary *params = @{@"action":@"doSp",@"table":@"cgd",@"data":[NSString stringWithFormat:@"{\"SPresult\":\"%zi\",\"SPcontent\":\"%@\",\"recordid\":\"%@\"}",status,_pifuneirong.text,_model.Id]};
        [DataPost requestAFWithUrl:urlStr params:params finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
         ...
         */
        NSString *urlStr = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/sp"];
        NSDictionary *params = @{@"action":@"doSp",@"table":@"cgd",@"data":[NSString stringWithFormat:@"{\"recordid\":\"%@\",\"tpaccountid\":\"%@\",\"tpname\":\"%@\",\"SPresult\":\"%zi\",\"SPcontent\":\"%@\"}",_model.Id,_tPID,_pifurenname.titleLabel.text,status,_pifuneirong.text]};
        [DataPost requestAFWithUrl:urlStr params:params finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
            NSString *str1 = [[NSString alloc]initWithData:result encoding:NSUTF8StringEncoding];
            NSLog(@"批复结果返回:%@",str1);
            NSString *realStr;
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CgInforProductCell* procell = [tableView dequeueReusableCellWithIdentifier:@"CgInforProductCell"];
    if (!procell) {
        procell = [[[NSBundle mainBundle]loadNibNamed:@"CgInforProductCell" owner:self options:nil]firstObject];
    }
...
- (void)DataRequest2
{
    ...
    NSDictionary* parmas = @{@"mobile":@"true",@"action":@"getPurchaseDetail",@"table":@"cgmx",@"params":[NSString stringWithFormat:@"{\"purchaseidEQ\":\"%@\"}",_chanpinID]};
    ...
    [DataPost requestAFWithUrl:urlStr params:parmas finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
        ...
        if ([returnStr isEqualToString:@"sessionoutofdate"]) {
            ...
            for (NSDictionary *dic in  arr) {
                CgInformationDetailModel *model = [[CgInformationDetailModel alloc] init];
                [model setValuesForKeysWithDictionary:dic];
            ...
            _tbView.frame = CGRectMake(0, 10+45*9, KscreenWidth, 315*_dataArray.count);
            if ([self.vcType isEqualToString:@"1"]) {
                _spView.frame = CGRectMake(0, _tbView.bottom+10, KscreenWidth, 180+45*pifuLishicount);
                self.dingDanScrollView.contentSize = CGSizeMake(KscreenWidth, 10+45*9+315*_dataArray.count+20+_spView.height);
            }else{
                self.dingDanScrollView.contentSize = CGSizeMake(KscreenWidth, 10+45*9+315*_dataArray.count+20);
            }
            [_tbView reloadData];
            NSLog(@"产品信息%@",arr);
        }
