...
#import "DataPost.h"
#import "UIViewExt.h"
#import "ProPlanLiuLanVC.h"
#import "ProPlanSearchViewController.h"
@interface ProductPlanViewController ()<UITextFieldDelegate>
...
@implementation ProductPlanViewController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"生产计划管理";
...
//    }
    ...
    if (_liuLanButton.selected == YES) {
        urlStr = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/production?action=getPlanPhoneBeans"];
        params = @{@"rows":@"10",@"page":[NSString stringWithFormat:@"%zi",_searchPage1],@"action":@"getPlanPhoneBeans",@"params":[NSString stringWithFormat:@"{\"table\":\"scjh\",\"plannoLIKE\":\"%@\",\"proidEQ\":\"\",\"creatoridEQ\":\"\",\"isvalidEQ\":\"1\",\"plantimeGE\":\"%@\",\"plantimeLE\":\"%@\"}",_orederNo,_start,_end]};
        [DataPost requestAFWithUrl:urlStr params:params finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
            ...
            for (NSDictionary *dic in array) {
                ProPlanListModel *model = [[ProPlanListModel alloc] init];
         ...
         */
        urlStr = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/production?action=getSPBeans"];
        params = @{@"rows":@"10",@"page":[NSString stringWithFormat:@"%zi",_searchPage2],@"action":@"getSPBeans",@"params":[NSString stringWithFormat:@"{\"table\":\"scjh\",\"plannoLIKE\":\"%@\",\"creatoridEQ\":\"\",\"isspEQ\":\"0\",\"plantimeGE\":\"%@\",\"plantimeLE\":\"%@\"}",_orederNo,_start,_end]};
        [DataPost requestAFWithUrl:urlStr params:params finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
            NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:result options:NSJSONReadingMutableContainers error:nil];
            NSArray *array = dic[@"rows"];
            NSLog(@"搜索上传字符串%@",params);
...
{
    /*生产浏览列表接口
     http://192.168.1.199:8080/jingxin/servlet/production?action=getPlanPhoneBeans
     参数：{"table":"scjh","plannoLIKE":"","proidEQ":"","creatoridEQ":"","isvalidEQ":"1","plantimeGE":"","plantimeLE":""}
     */
    //进度HUD
    MBProgressHUD* hud = [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    ...
    hud.labelText = @"网络不给力，正在加载中...";
    [hud show:YES];
    NSString *urlStr = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/production?action=getPlanPhoneBeans"];
    NSDictionary *params = @{@"rows":@"10",@"page":[NSString stringWithFormat:@"%zi",_page1],@"action":@"getPlanPhoneBeans",@"params":[NSString stringWithFormat:@"{\"table\":\"scjh\",\"plannoLIKE\":\"\",\"proidEQ\":\"\",\"creatoridEQ\":\"\",\"isvalidEQ\":\"1\",\"plantimeGE\":\"%@\",\"plantimeLE\":\"%@\"}",_pastDateStr,_currentDateStr]};
    NSLog(@">>>>%@",urlStr);
    [DataPost requestAFWithUrl:urlStr params:params finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
        if (_page1 == 1) {
...
- (void)DataRequest1
{
    ...
    hud.labelText = @"网络不给力，正在加载中...";
    [hud show:YES];
    NSString *urlStr = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/production?action=getSPBeans"];
    NSDictionary *params = @{@"action":@"getSPBeans",@"rows":@"20",@"page":[NSString stringWithFormat:@"%zi",_page2],@"params":[NSString stringWithFormat:@"{\"table\":\"scjh\",\"plannoLIKE\":\"\",\"creatoridEQ\":\"\",\"isspEQ\":\"0\",\"plantimeGE\":\"%@\",\"plantimeLE\":\"%@\"}",_pastDateStr,_currentDateStr]};
    [DataPost requestAFWithUrl:urlStr params:params finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
        if (_page2 == 1) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    PPLiuLanCell *cell = [tableView dequeueReusableCellWithIdentifier:@"PPLiuLanCellID"];
    if (cell == nil) {
        cell = (PPLiuLanCell *) [[[NSBundle mainBundle] loadNibNamed:@"PPLiuLanCell" owner:self options:nil]firstObject];
    }
    ...
    if (tableView == self.liuLanTableView)
    {
        ...
        [cell setTransValue:^(BOOL isClick,NSString* orderID) {
            [self aleartView:orderID];
        }];
        return cell;
    }else if(tableView == self.shenPiTableView){
        if (_searchFlag == 1) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == self.liuLanTableView)
    {
        ProPlanLiuLanVC *liuLanMessage =[[ProPlanLiuLanVC alloc] init];
        liuLanMessage.vcType = @"0";
        if (_searchFlag == 1) {
            if (_searchDateArray1.count != 0) {
                ProPlanListModel *model = [_searchDateArray1 objectAtIndex:indexPath.row];
                liuLanMessage.model = model;
                liuLanMessage.chanpinID = model.Id;
            }
        }else{
            if (_dataArray1.count != 0) {
                ProPlanListModel *model = [_dataArray1 objectAtIndex:indexPath.row];
                liuLanMessage.model = model;
                liuLanMessage.chanpinID = model.Id;
            }
        }
    ...
    {
        ProPlanLiuLanVC *shenPiMessage =[[ProPlanLiuLanVC alloc] init];
        shenPiMessage.vcType = @"1";
        if (_searchFlag == 1) {
            if (_searchDateArray2.count != 0) {
                ProPlanListModel *model = [_searchDateArray2 objectAtIndex:indexPath.row];
                shenPiMessage.model = model;
                shenPiMessage.spStatus = [model.spstatus integerValue];
                shenPiMessage.chanpinID = model.Id;
                shenPiMessage.orderNo = model.planno;
            }
        }else{
            if (_dataArray2.count != 0) {
                ProPlanListModel *model = [_dataArray2 objectAtIndex:indexPath.row];
                shenPiMessage.model = model;
                shenPiMessage.spStatus = [model.spstatus integerValue];
                shenPiMessage.chanpinID = model.Id;
                shenPiMessage.orderNo = model.planno;
    ...
    }else if(tableView == self.classTableView)
    {
        if (indexPath.row == 0) {
            ...
            AddAnotherPlanVC *tianjiaVC = [[AddAnotherPlanVC alloc] init];
            [self.navigationController  pushViewController:tianjiaVC animated:YES];
            [self.m_keHuPopView removeFromSuperview];
        }else{
            
            AddProPlanDingdanVC *tianjiaVC = [[AddProPlanDingdanVC alloc] init];
            [self.navigationController  pushViewController:tianjiaVC animated:YES];
            [self.m_keHuPopView removeFromSuperview];
        }
    }
...
- (void)addNext{
    ...
    ProPlanSearchViewController *sousuoVC = [[ProPlanSearchViewController alloc] init];
...
- (void)searchAction{
    [self takeSelectAction];
//    AddProPlanDingdanVC *tianjiaVC = [[AddProPlanDingdanVC alloc] init];
//    [self.navigationController  pushViewController:tianjiaVC animated:YES];
}
- (void)takeSelectAction{
    
    //_classButton.userInteractionEnabled = NO;
    _classArray = @[@"订单申请计划下达",@"生产计划下达"];
    ...
    UILabel* typeLabel = [[UILabel alloc]initWithFrame:CGRectMake(10, 5, 100, 20)];
    typeLabel.text = @"计划下达方式";
...
- (void)aleartView:(NSString*)orderID{
    UIAlertController * alertController = [UIAlertController alertControllerWithTitle:@"操作："
                                                                              message:@""
                                                                       preferredStyle:UIAlertControllerStyleActionSheet];
    
    UIAlertAction * bomAction = [UIAlertAction actionWithTitle:@"生成BOM单" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [self createBOMRequest:orderID];
    }];
    [alertController addAction:bomAction];
    
    UIAlertAction * okAction = [UIAlertAction actionWithTitle:@"生产入库结束" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [self productinEndRequset:orderID];
    }];
...
/*
 /production?action=createBOM
 */
- (void)createBOMRequest:(NSString*)orderID{
    //进度HUD
    MBProgressHUD* hud = [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    //设置模式为进度框形的
    hud.mode = MBProgressHUDModeIndeterminate;
    hud.labelText = @"网络不给力，正在加载中...";
    [hud show:YES];
    NSString *urlStr = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/production?action=createBOM"];
    
    NSDictionary *params = @{@"action":@"createBOM",@"data":[NSString stringWithFormat:@"{\"id\":\"%@\"}",orderID]};
    NSLog(@">>>>%@",urlStr);
    [DataPost requestAFWithUrl:urlStr params:params finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
...
//            UIAlertController * alertController = [UIAlertController alertControllerWithTitle:realStr message:@"" preferredStyle:UIAlertControllerStyleAlert];
//            [self presentViewController:alertController animated:YES completion:nil];
            if ([realStr rangeOfString:@"true"].location != NSNotFound) {
                [self showAlert:@"生成成功"];
                [self newRefresh];
            }else if ([realStr rangeOfString:@"false"].location!=NSNotFound){
                [self showAlert:@"生成失败"];
            }else{
...
- (void)productinEndRequset:(NSString*)orderID{
    /*
     /production?action=productionFinished
     参数：id
     */
    //进度HUD
    MBProgressHUD* hud = [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    ...
    hud.labelText = @"网络不给力，正在加载中...";
    [hud show:YES];
    NSString *urlStr = [NSString stringWithFormat:@"%@%@",DATA_ADDRESS,@"/production?action=productionFinished"];
    
    NSDictionary *params = @{@"action":@"productionFinished",@"data":[NSString stringWithFormat:@"{\"id\":\"%@\"}",orderID]};
    NSLog(@">>>>%@",urlStr);
    [DataPost requestAFWithUrl:urlStr params:params finishDidBlock:^(AFHTTPRequestOperationManager *operation, id result) {
